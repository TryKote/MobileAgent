...
public final class MrimAccount extends Account implements ListItem {
    ...
    public MrimAccount(int accountId, String login, String password) {
        ...
        this.lastError = 0;
        this.configFlags = 1;
        MrimContactGroup defaultGrp = new MrimContactGroup(this, -1, GROUP_DEFAULT, StringPool.get(StringResKeys.STR_GROUP_DEFAULT));
    ...
    public final ContactGroup createOnlineGroup() {
        return new MrimContactGroup(this, -1, GROUP_NOT_IN_LIST, StringPool.get(StringResKeys.STR_GROUP_NOT_IN_LIST));
    }

    @Override
    public final ContactGroup createBlockedGroup() {
        return new MrimContactGroup(this, -1, GROUP_TEMPORARY, StringPool.get(StringResKeys.STR_GROUP_TEMPORARY));
    }

    @Override
    public final ContactGroup createOfflineGroup() {
        return new MrimContactGroup(this, -1, GROUP_BLOCKED, StringPool.get(StringResKeys.STR_GROUP_IGNORE));
    }

    @Override
    public final ContactGroup createSpecialGroup() {
        return new MrimContactGroup(this, -1, GROUP_PHONE_CONTACTS, StringPool.get(StringResKeys.STR_GROUP_PHONE_CONTACTS));
    }
    ...
    private void handleHelloAck(ByteBuffer packet) throws Throwable {
        ...
        sendData(ProtocolFactory.createMrimPacket(this, MrimCommand.CS_LOGIN2, authPacket.writeIntLE(useExtended ? -1 : FEATURE_VERSION_BASIC).writeStringLatin1(useExtended ? null : new ByteBuffer().writeCharBytes("client=\"jagent\" version=\"").writeExtendedInt(2229599).getStringAndClear()).writeCharBytes(StringPool.get(PackedStringKeys.MRIM_GEO_LIST_PACKET)).writeStringLatin1(XmppContactGroup.buildAuthData()).writeBuffer(XmppContactGroup.buildSyncPayload(this))));
            ...
            while (pos < length) {
                ...
                if (!parsingValue) {
                    if (ch == '\n' && lineBuffer.length() == 0) {
                        ...
                        String typeCodeStr = (String) headers.get(StringPool.get(PackedStringKeys.HEADER_X_MRIM_MULTICHAT));
                        int typeCode = typeCodeStr != null ? -1 : Integer.parseInt(typeCodeStr);
                        messageType = typeCode;
                        senderName = typeCode >= 0 ? null : Base64.decode(StringUtils.suffix((String) headers.get(StringPool.get(PackedStringKeys.MAIL_FIELD_SUBJECT)), 13)).readAllWideStr();
                        headerRef = (String) headers.get(StringPool.get(PackedStringKeys.MAIL_FIELD_SENDER));
                        messageFlags = Integer.parseInt((String) headers.get(StringPool.get(PackedStringKeys.HEADER_X_MRIM_FLAGS)), 16);
                        timestamp = Utils.parseDateTime((String) headers.get(StringPool.get(PackedStringKeys.MAIL_FIELD_DATE)));
                        encodingType = 1;
    ...
    private void handleProfileData(ByteBuffer packet) {
        ...
        if (AccountManager.getTotalSyncCount() == 10) {
            EventDispatcher.postNotification(StringPool.get(StringResKeys.STR_MRIM_DISCONNECT));
        }
    }
    ...
    public final int setConfiguration(int statusCode) {
        ...
        if (statusTextId >= 0) {
            ...
            statusText = ObjectPool.toStringAndRelease(ObjectPool.newStringBuffer().append(StringPool.get(StringResKeys.STR_CONFIG_STATUS_PREFIX)).append(this.configFlags >> 8));
        }
        switch (this.configFlags) {
            case STATUS_ONLINE:
                typeStr = StringPool.get(StringResKeys.STR_STATUS_ONLINE);
                break;
            case STATUS_DND:
                typeStr = StringPool.get(StringResKeys.STR_STATUS_DND);
                break;
            case STATUS_FREE_CHAT:
                typeStr = StringPool.get(StringResKeys.STR_STATUS_ONLINE);
                break;
            case STATUS_AWAY:
                typeStr = StringPool.get(StringResKeys.STR_STATUS_AWAY);
                break;
            case STATUS_INVISIBLE:
                typeStr = StringPool.get(StringResKeys.STR_STATUS_INVISIBLE);
                break;
            default:
                typeStr = StringPool.get(StringResKeys.STR_CONFIG_TYPE_BASE + (this.configFlags >> 8));
                break;
        }
    ...
    public final String getText() {
        ...
        if (this.profileManager.profile.dirty) {
            sb.append(StringPool.get(StringResKeys.STR_MRIM_AWAY_SUFFIX));
            ...
            sb.append(StringPool.get(StringResKeys.STR_MRIM_OFFLINE_SUFFIX));
        ...
        if (showInList || showPopup) {
            ...
            if (showPopup && (AccountManager.getTotalSyncCount() != 10 || !AppState.hasMemory())) {
                ...
                if (str2 != null && str != null) {
                    EventDispatcher.postAccountNotification(this, ObjectPool.toStringAndRelease(sb.append(StringPool.get(StringResKeys.STR_NEW_MAIL_FROM)).append(str).append(' ').append('\"').append(str2).append('\"').append('.').append('\n').append(new StringBuffer().append(i > 0 ? new StringBuffer().append(StringPool.get(StringResKeys.STR_NEW_MAIL_COUNT)).append(i).append(StringPool.get(StringResKeys.STR_NEW_MAIL_SUFFIX + Utils.pluralForm(i))).append('\n').toString() : AppState.emptyStr).append(StringPool.get(StringResKeys.STR_MAIL_PREFIX)).toString())));
                } else if (i > 0) {
                    EventDispatcher.postAccountNotification(this, ObjectPool.toStringAndRelease(sb.append(StringPool.get(StringResKeys.STR_NEW_MAIL_COUNT)).append(i).append(StringPool.get(StringResKeys.STR_NEW_MAIL_SUFFIX + Utils.pluralForm(i))).append('\n').append(StringPool.get(StringResKeys.STR_MAIL_PREFIX))));
                }
            }
            if (showInList) {
    ...
    public int sendSmsMessage(Contact contact, String phone, String message) {
        ...
        contact.appendMessage(1, ObjectPool.toStringAndRelease(
                Utils.appendColon(ObjectPool.newStringBuffer()
                        .append(StringPool.get(StringResKeys.STR_FILE_TRANSFER_PREFIX))
